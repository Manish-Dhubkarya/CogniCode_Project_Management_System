...
const HeadClientProjectInfo:React.FC<ClientProjectInfoProps> = ({EmployeeDesignation, EmployeeName, ProjectID, ProjectTitle, AboutCompany, WhatDoYouNeed, ChatMessages}) => {
  ...
  return (
    <div className={`flex flex-col w-full text-black  ${isLG?"px-[7.5vw] py-20 overflow-y-auto min-h-screen justify-center": (isXL || is2XL)?"px-[10.5vw] min-h-screen overflow-y-auto py-20 justify-center": "px-[4vw] py-15"}  items-center`}>
      ...
      <div className="w-full space-x-[8vw] flex">
         <Button1 value={EmployeeDesignation} gradientType="gradient1" />
        <div className="w-fit">
        <ProgressTracking2/>
        </div>
      </div>
      <div className={`w-full ${isLG || isXL || is2XL?"flex-row space-x-15 ":"flex-col space-y-15"} flex justify-between  items-center`}>
        {/* Left Column */}
        <div className={`${isLG || isXL || is2XL?"w-[55%]":"w-full"} ${is2XL?"text-[14px]":"text-[12px]"} flex flex-col space-y-4 items-start`}>
        ...
        {/* Right Column */}
        <div className={`${isLG || isXL || is2XL?"w-[45%]":"w-full"} px-3 min-h-[500px] space-y-7 flex flex-col items-center justify-between pb-10 bg-[#CFE3FF] rounded-[10px]`}>
